...
export class KeycloakAuthorizationService {
    ...
    //private _authinstance: KeycloakAuth.KeycloakAuthorizationInstance;
    //private _kcInstance: Keycloak.KeycloakInstance;
    //private kcConfig:Promise<any> = null;
   ...
   * It is possible to check for the authorization on a non-default resource-server,
   * and to force the re-loading of entitlements from keycloak  
   * 
   ...
   * Name of the resource server to check. If not present use default resource-server
   * Only needed when refreshing entitlements from keycloak
   * 
   * @param forceRefresh
   * Defines if entitlements are refreshed from keycloak. Default is false
   * 
   * @returns boolean true if user has access, false if not
   */
  public async checkAuthorization(authorization,resourceServerId?:string,forceRefresh?:boolean) {
      
      if(t(resourceServerId).isUndefined){
          resourceServerId = this._defaultResourceServerId;
      }
      let checkResource = t(authorization,'rsname').safeObject;
      let checkScope = t(authorization,'scope').safeObject;
      ...
      if (forceRefresh){
          ...
          await this.getAuthorizations(resourceServerId,authorizationRequest);
      }
   ...
   * @returns boolean true if user has access, false if not
   */
  private hasAuthorization(authorization):boolean{
   ...
   */
  public getAuthorizations(resourceServerId, authorizationRequest) {
   ...
   */
  private getEntitlement(resourceServerId, authorizationRequest) {
